...
class UserRegisterView(generics.CreateAPIView):
    ...
    @extend_schema(
        request=RegisterSerializer,
        responses={201: ProfileSerializer},
        description="Register new user with role: client, staff, manager, admin"
    )
    def post(self, request, *args, **kwargs):
        return self.create(request, *args, **kwargs)
...
# -------------------------------------------------------------------
# 2. LOGIN VIEW (ADDED - REQUIRED FOR YOUR FRONTEND)
# -------------------------------------------------------------------
class UserLoginView(TokenObtainPairView):
    """
    Returns access, refresh, AND user role:
    {
       "access": "...",
       "refresh": "...",
    ...
    """
    permission_classes = [AllowAny]
    serializer_class = LoginSerializer   # ← includes role
...
# -------------------------------------------------------------------
# 5. USER VIEWSET (register + profile)
# -------------------------------------------------------------------
class UserViewSet(viewsets.ModelViewSet):
    queryset = User.objects.all()
    serializer_class = UserSerializer
